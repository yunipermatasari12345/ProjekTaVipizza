...
export function AuthProvider({ children }) {
  ...
  useEffect(() => {
    ...
    fetch('https://8a49cf3c307c57.lhr.life/api/auth/me', {
  ...
  const loginSimulasi = async (email, password) => {
    try {
      const response = await fetch('https://8a49cf3c307c57.lhr.life/api/auth/login', {
        method: 'POST',
  ...
  const registerSimulasi = async (nama, email, telepon, alamat, password = "pelangganvipizza") => {
    try {
      const response = await fetch('https://8a49cf3c307c57.lhr.life/api/auth/register', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
