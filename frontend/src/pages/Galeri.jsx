...
export default function Galeri() {
  ...
  const muatGaleri = () => {
    fetch(`http://${window.location.hostname}:9000/api/galeri`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/galeri`, {
        method: 'POST',
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 md:gap-6">
            {galeri.map((item) => (
              ...
              >
                <img 
                  src={`http://${window.location.hostname}:9000${item.gambar_url}`} 
                  alt={item.judul}
      ...
      {selectedImage && (
        ...
        >
          ...
          <div className="max-w-4xl w-full flex flex-col items-center animate-in zoom-in-95 duration-300" onClick={e => e.stopPropagation()}>
            <img 
              src={`http://${window.location.hostname}:9000${selectedImage.gambar_url}`} 
