...
export default function Beranda() {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      fetch(`http://${window.location.hostname}:9000/api/ulasan/terbaru`)
        .then(r => r.json())
        .then(data => {
    ...
    fetch(`http://${window.location.hostname}:9000/api/promo`)
      .then(r => r.json())
  ...
  const handleKirimTesti = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`http://${window.location.hostname}:9000/api/ulasan/publik`, {
        method: 'POST',
  ...
  const handleHapusUlasan = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`http://${window.location.hostname}:9000/api/ulasan/${id}`, {
          method: 'DELETE',
