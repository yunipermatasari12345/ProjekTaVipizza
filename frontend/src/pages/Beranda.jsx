...
export default function Beranda() {
  ...
  useEffect(() => {
    ...
    const fetchData = () => {
      ...
      fetch('https://8a49cf3c307c57.lhr.life/api/ulasan/terbaru')
        .then(r => r.json())
        .then(data => {
  ...
  const handleKirimTesti = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://8a49cf3c307c57.lhr.life/api/ulasan/publik', {
        method: 'POST',
  ...
  const handleHapusUlasan = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        const response = await fetch(`https://8a49cf3c307c57.lhr.life/api/ulasan/${id}`, {
          method: 'DELETE',
