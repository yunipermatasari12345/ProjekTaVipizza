...
export default function TrackOrder() {
  ...
  useEffect(() => {
    const muatPesanan = async () => {
      ...
      if (token) {
        try {
          const response = await fetch(`https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/${id}`, {
            headers: {
          ...
          if (response.ok) {
            ...
            if (data && data.id) {
              ...
              pesananDitemukan = {
                ...
                bukti_pembayaran: data.bukti_pembayaran ? `https://optimum-setting-incidence-barn.trycloudflare.com${data.bukti_pembayaran}` : "",
  ...
  const handleRefreshToken = async () => {
    ...
    try {
      const response = await fetch(`https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/${id}/refresh-token`, {
        method: 'POST',
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await response.json();
      if (response.ok && data.snap_token) {
        ...
        if (window.snap) {
          window.snap.pay(data.snap_token, {
            onSuccess: function() { 
              fetch('https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/' + id + '/verify-payment', {
                method: 'POST',
                headers: { 'Authorization': 'Bearer ' + token }
              }).catch(function(){});
  ...
  const handleCekStatusPembayaran = async () => {
    ...
    try {
      const response = await fetch('https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/' + id + '/verify-payment', {
        method: 'POST',
        headers: {
          'Authorization': 'Bearer ' + token
        }
  ...
  const handleUploadBukti = async (e) => {
    ...
    if (token && token !== "mock_jwt_token_vipizza" && fileBukti) {
      try {
        ...
        const response = await fetch(`https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/${id}/payment`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${token}`
          },
        ...
        if (response.ok) {
          ...
          if (res && res.bukti_pembayaran_url) {
            finalGambarURL = `https://optimum-setting-incidence-barn.trycloudflare.com${res.bukti_pembayaran_url}`;
  ...
  const handleKirimUlasan = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`https://optimum-setting-incidence-barn.trycloudflare.com/api/orders/${id}/ulasan`, {
        method: 'POST',
        headers: {
