...
export default function Reports() {
  ...
  const hitungLaporan = async () => {
    try {
      ...
      const response = await fetch(`http://${window.location.hostname}:9000/api/reports/sales?${params.toString()}`, {
        method: 'GET',
        headers: {
  ...
  const handleUnduhPDF = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://${window.location.hostname}:9000/api/reports/pdf?${params.toString()}`, {
        method: 'GET',
        headers: {
          Authorization: `Bearer ${token}`,
        },
