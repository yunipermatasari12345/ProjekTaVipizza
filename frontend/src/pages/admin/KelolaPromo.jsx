...
export default function KelolaPromo() {
  ...
  const muatPromo = async () => {
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/promo/admin`, {
  ...
  const handleBukaEdit = (p) => {
    setPromoTerpilih(p);
    setSelectedFile(null);
    setPreviewUrl(p.banner_url ? `http://${window.location.hostname}:9000${p.banner_url}` : null);
    setFormData({
  ...
  const handleSimpanPromo = async (e) => {
    ...
    const apiURL = promoTerpilih ? `http://${window.location.hostname}:9000/api/promo/${promoTerpilih.id}` : `http://${window.location.hostname}:9000/api/promo`;
  ...
  const handleHapusPromo = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`http://${window.location.hostname}:9000/api/promo/${id}`, {
            method: 'DELETE',
            headers: { Authorization: `Bearer ${token}` },
          });
                    ...
                    promos.map((p, index) => {
                      ...
                      <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${kadaluarsa ? 'opacity-60 bg-red-50/30' : ''}`}>
                        ...
                        <td className="py-3 px-4 text-center">
                          {p.banner_url ? (
                            <img src={`http://${window.location.hostname}:9000${p.banner_url}`} alt={p.judul} className="w-16 h-10 object-cover rounded border border-gray-200 mx-auto" />
                          ) : (
