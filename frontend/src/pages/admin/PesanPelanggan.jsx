...
export default function PesanPelanggan() {
  ...
  const muatPesan = async (isAutoRefresh = false) => {
    ...
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/pesan-pelanggan`, { headers });
      if (res.ok) {
  ...
  const handleBalas = async () => {
    ...
    let berhasil = false;
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/pesan-pelanggan/${pesanId}/balas`, {
        method: 'PUT',
        headers,
  ...
  const handleHapus = (id) => {
    Swal.fire({
      ...
      try {
        await fetch(`http://${window.location.hostname}:9000/api/pesan-pelanggan/${id}`, { method: 'DELETE', headers });
      } catch { /* ignore */ }
