...
export default function Dashboard() {
  const { token } = useAuth();
  const API = `http://${window.location.hostname}:9000/api`;
          ...
          <div className="bg-white rounded-xl w-full max-w-lg p-6 shadow-xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
            ...
            {detail.bukti_pembayaran && (
              <div className="mb-4">
                ...
                <img src={detail.bukti_pembayaran.startsWith('http') ? detail.bukti_pembayaran : `http://${window.location.hostname}:9000${detail.bukti_pembayaran}`} alt="Bukti" className="w-full max-h-48 object-contain rounded-lg border border-gray-200" />
              </div>
            )}
