...
export default function KelolaPesanan() {
  ...
  const muatPesanan = useCallback(async () => {
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/orders`, {
  ...
  const handleUpdateStatus = async (id, statusBaru) => {
    try {
      const res = await fetch(`http://${window.location.hostname}:9000/api/orders/${id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeletePesanan = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`http://${window.location.hostname}:9000/api/orders/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (res.ok) {
