...
export default function KelolaPesanan() {
  ...
  const muatPesanan = useCallback(async () => {
    try {
      const res = await fetch('https://8a49cf3c307c57.lhr.life/api/orders', {
  ...
  const handleUpdateStatus = async (id, statusBaru) => {
    try {
      const res = await fetch(`https://8a49cf3c307c57.lhr.life/api/orders/${id}/status`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeletePesanan = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const res = await fetch(`https://8a49cf3c307c57.lhr.life/api/orders/${id}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${token}` }
          });
          if (res.ok) {
